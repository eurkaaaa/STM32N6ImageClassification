...
/* USER CODE BEGIN PV */
volatile int32_t nor_write_ret, nor_read_ret, nor_clean_ret, nor_mmap_ret;
volatile int32_t ram_write_ret, ram_read_ret, ram_mmap_ret;
uint8_t ram_rx[256];

const uint8_t *image = (const uint8_t *)stlogo;
...
/*  while(1)
  {
	  ...
	  const uint32_t offset = 0x1000U;
	  const volatile uint8_t *psram_ptr = (const volatile uint8_t *)(XSPI1_BASE + offset);
	  const volatile uint8_t *flash_ptr = (const volatile uint8_t *)(XSPI2_BASE + offset);
	  ...
	  if (done == 0U)
	  {
	    ...
	    (void)BSP_XSPI_NOR_DisableMemoryMappedMode(0);

	    nor_write_ret = BSP_XSPI_NOR_Write(0, write_data, offset, sizeof(write_data));
	    nor_read_ret = BSP_XSPI_NOR_Read(0, read_data, offset, sizeof(read_data));
	    nor_clean_ret = BSP_XSPI_NOR_Erase_Block(0, offset, BSP_XSPI_NOR_ERASE_4K);
	    nor_mmap_ret = BSP_XSPI_NOR_EnableMemoryMappedMode(0);

	    (void)memcpy((void*)ram_rx, (const void *)flash_ptr, sizeof(ram_rx));


	    uint8_t read_data1[4] = {0};

	    (void)BSP_XSPI_RAM_DisableMemoryMappedMode(0);

	    ram_write_ret = BSP_XSPI_RAM_Write(0, write_data, offset, sizeof(write_data));
	    ram_read_ret = BSP_XSPI_RAM_Read(0, read_data1, offset, sizeof(read_data1));
	    ram_mmap_ret = BSP_XSPI_RAM_EnableMemoryMappedMode(0);

	    (void)memcpy((void*)ram_rx, (const void *)psram_ptr, sizeof(ram_rx));
